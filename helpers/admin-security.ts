...
export const adminBootstrapConfig = {
  ...
  ipWhitelist: process.env.ADMIN_SETUP_IP_WHITELIST?.split(",").map((ip) =>
    ip.trim(),
  ),
...
export function validateBootstrapAccess(requestIp?: string): {
  ...
  if (!adminBootstrapConfig.isEnabled) {
    return {
      allowed: false,
      reason:
        "Admin setup is disabled. Set ADMIN_SETUP_ENABLED=true to enable.",
    };
  }

  // Prevent bootstrap in production unless explicitly allowed
  if (
    adminBootstrapConfig.isProduction &&
    !process.env.ADMIN_SETUP_ALLOW_PRODUCTION
  ) {
    return {
      allowed: false,
...
export function validateSetupSecret(providedSecret: string): boolean {
  ...
  for (let i = 0; i < providedSecret.length; i++) {
    result |=
      providedSecret.charCodeAt(i) ^
      adminBootstrapConfig.setupSecret.charCodeAt(i);
  }
